...
扫描结果表格列：保留 / 重复组 / 文件名 / 文件大小 / 文件路径。

- **打开文件**：点击“文件名”单元格，用系统默认程序打开该文件；
- **定位文件**：点击“文件路径”单元格，在系统资源管理器中选中该文件；若该文件已删除（如已移入回收站），则改为打开其所在目录，便于核对删除结果；
- **保留与删除**：勾选“保留”列表示要保留该文件（默认全部不勾选）；点击「删除未勾选文件」按钮后，其余未勾选文件会被移入系统回收站，删除前会二次确认并提示整组清空的组数。删除由 Electron 主进程调用 `shell.trashItem` 完成，路径严格限定在最近一次扫描的目录内；单个文件失败会单独列出且不影响其余文件。删除后**表格保持原样不刷新**，方便按路径逐条核对；已成功删除的文件不会在再次点击删除时重复提交。
